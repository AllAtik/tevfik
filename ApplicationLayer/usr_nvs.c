...
extern S_USR_NVS_DEVICE_INFO_BOOT_DATA g_sUsrSystemNvsDeviceInfoBootData;
...
// void UsrNvsBootDataUpdtae(void)

_io void CheckFactorySettingProc(void)
{
    ReadDataToEepromProc(_USR_NVS_START_ADD, (uint8_t *)&g_sNvsDeviceInfo, sizeof(g_sNvsDeviceInfo));
    // ReadDataToEepromProc(_USR_EEPROM_START_ADD, (uint8_t*)&g_sUsrSystemNvsDeviceInfoBootData, sizeof(g_sUsrSystemNvsDeviceInfoBootData));
    ...
    if ((g_sNvsDeviceInfo.eepromCheckValue != _USR_NVS_CHECKDATA) || (CalculateCrcProc((uint8_t *)&g_sNvsDeviceInfo, sizeof(g_sNvsDeviceInfo) - 1) != g_sNvsDeviceInfo.crc))
    {
        ...
        g_sNvsDeviceInfo.sendingDataInterval = (uint32_t)60;  // seconds
        ...
        g_sNvsDeviceInfo.sensorWakeUpTime = (uint32_t)60 * 3; // seconds
        g_sNvsDeviceInfo.accReadingTime = (uint32_t)60;       // seconds
        g_sNvsDeviceInfo.crc = 0;
        g_sNvsDeviceInfo.deviceStatusCheckTime = (uint32_t)60 * 2; // seconds
        g_sNvsDeviceInfo.deviceStatus = 1;                         // 0                             // flag
        g_sNvsDeviceInfo.dailyReset = 10000;                       // 0

        g_sNvsDeviceInfo.accHardwareOkflag = 0;
