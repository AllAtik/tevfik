...
void UL_NtcPeripheral(S_NTC_PARAMETERS *pntc, EControl f_eControl)
{
  m_sNtcParameters = *pntc;
  if(f_eControl == disableNtcPeripheral)
  {
    m_ntcpowerokflag = false;
    UsrSleepSensorGpioPins(m_sNtcParameters.pNtcPort, m_sNtcParameters.pNtcPin, (GPIO_PinState)!m_sNtcParameters.ntcstatus);
  }
  else
  {
    m_ntcpowerokflag = true;
    UsrWakeUpSensorGpioPins(m_sNtcParameters.pNtcPort, m_sNtcParameters.pNtcPin, (GPIO_PinState)m_sNtcParameters.ntcstatus);
  }
}
...
{
  float g_ntcraw = ((_NTC_UP_R) / ((_USR_ADC_RESOLUTION /analogValue) - 1));        // 12 bit resolution
...
float UL_NtcGetValue(uint32_t f_raw)
{
  uint8_t count = 0;
  float resistor, resmin, resmax;
  float retval, step, stepcount;

  resistor = (_NTC_SERIAL_RES * f_raw) / (_USR_ADC_RESOLUTION - f_raw); 
